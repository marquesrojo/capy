...
export default function CamautRegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      // 3. Crear registro en staff_names
  ...
  if (step === 3) {
    return (
      <div className="min-h-screen bg-[#F0F4F8] flex flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="font-bold text-[#1A2A3A] text-2xl mb-2">Revisá tu email</h1>
        <p className="text-[#8896A5] text-sm mb-6 max-w-xs">
          Te enviamos un link de confirmación a <strong>{email}</strong>. Hacé click en el link para activar tu cuenta.
        </p>
        <Link to="/camaut/login" className="text-[#008080] text-sm underline">
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8] px-6 py-10">
      ...
      <div className="mt-6 mb-8">
        <h1 className="font-bold text-[#1A2A3A] text-2xl mb-1">
        ...
        <div className="flex gap-2 mt-3">
          <div className="flex-1 h-1.5 rounded-full bg-[#008080]" />
          <div className={`flex-1 h-1.5 rounded-full ${step === 2 ? 'bg-[#008080]' : 'bg-black/10'}`} />
        </div>
      </div>
      ...
      {step === 1 ? (
        <form onSubmit={handleStep1} className="space-y-4">
          <div>
            <label className="text-[#8896A5] text-xs font-semibold uppercase tracking-wide block mb-1.5">
              Tu nombre completo
            </label>
            <input
              type="text"
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-black/10 rounded-xl px-4 py-3 text-sm bg-white text-[#1A2A3A]"
            />
            <p className="text-[#B0BEC5] text-[10px] mt-1">Los demás te ven con este nombre en el ranking</p>
          </div>
          <div>
            <label className="text-[#8896A5] text-xs font-semibold uppercase tracking-wide block mb-1.5">
              DNI / Documento
            ...
            <input
              ...
              className="w-full border border-black/10 rounded-xl px-4 py-3 text-sm bg-white text-[#1A2A3A]"
            />
            <p className="text-[#B0BEC5] text-[10px] mt-1">Necesario para generar tu certificado verificado</p>
          </div>
          <div>
            <label className="text-[#8896A5] text-xs font-semibold uppercase tracking-wide block mb-1.5">
              LinkedIn (opcional)
          ...
          <button
            ...
            disabled={loading}
            className="w-full text-[#8896A5] text-sm underline"
          >
            Saltar este paso
          </button>
        </form>
      ...
      <p className="text-[#8896A5] text-xs text-center mt-6">
        ¿Ya tenés cuenta?{' '}
        <Link to="/camaut/login" className="text-[#008080] underline">Iniciá sesión</Link>
      </p>
    </div>
  )
}
